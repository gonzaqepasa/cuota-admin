...
export default function JiuJitzu() {
  /////////////// BORRAR ///////////////
  ...
  async function getDataAgain() {
    setLoad(true);
    try {
      const res = await fetch(`${url}/user/get-users?activity=Jiu Jitzu`);
  ...
  useEffect(() => {
    (async function () {
      try {
        const resUser = await fetch(`${url}/user/get-users?activity=Jiu Jitzu`);
        const resAct = await fetch(
          `${url}/activity/get-activity?activity=Jiu Jitzu`
        );
  ...
  return (
    <div className={`main backg backg-jiu-jitzu`}>
      <Title activityName={"Jiu Jitzu"} />
      {!load && (
        <ButtonAdd setModalAdd={setModalAdd} color={"Jiu Jitzu"} />
      )}

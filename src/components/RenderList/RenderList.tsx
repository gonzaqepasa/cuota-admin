  ...
  return (
    ...
    >
      ...
      <Table
        ...
        shadow="md"
        className={`scroll max-w-3xl flex    w-11/12 overflow-y-auto h-[500px]  rounded-xl`}
      >
        <TableHeader>
          ...
          {orderByName(orderByActive(resultFilter)).map(
            (el: typesUser, index) => (
              <TableRow key={el._id}>
                <TableCell>
                  <Link
                    className="flex w-max items-center hover:translate-x-1 hover:text-neutral-600 font-medium transition text-neutral-900"
                    href={`/user/${el._id}`}
                  >
                    <Avatar
                      style={{
